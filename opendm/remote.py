...
class LocalRemoteExecutor:
    ...
    def run(self, taskClass):
        ...
        # Create queue thread
        local_thread = threading.Thread(target=local_worker) #start 2 threads
        if self.node_online:
...
class Task:
    ...
    def execute_remote_task(self, done, seed_files = [], seed_touch_files = [], outputs = [], ):
        ...
        # this uses pyodm sends a post method to /task/new/init to the node server
        # this also deals with uploads

        log.ODM_INFO("==================================")
        log.ODM_INFO("Create the Remote Task of %s with Upload Current Time is  %s" % (self , str(system.now_raw())))
        log.ODM_INFO("==================================")        
        task = self.node.create_task(images,
                ...
                outputs=outputs)
        log.ODM_INFO("==================================")
        log.ODM_INFO("Task Creation of %s and Upload is Complete Task with Upload Current Time is  %s" % (self ,str(system.now_raw())))
        log.ODM_INFO("==================================")
        ...
        # Check status
        info = task.info()
        custom_check_variable = False
        if info.status in [TaskStatus.RUNNING, TaskStatus.COMPLETED]:
            if info.status in [TaskStatus.RUNNING] and not custom_check_variable:
                log.ODM_INFO("==================================")
                log.ODM_INFO("The Current Running Time for %s with Current Time is  %s" % (self , str(system.now_raw())))
                log.ODM_INFO("==================================")
                custom_check_variable = True
            def monitor():
                class nonloc:
                ...
                try:
                    ...
                    log.ODM_INFO("==================================")
                    log.ODM_INFO("LRE: Download Assets for %s with and Current Time is  %s" % (self , str(system.now_raw())))
                    log.ODM_INFO("==================================")
                    ...
                    log.ODM_INFO("==================================")
                    log.ODM_INFO("LRE: Download Assets Finished and the Process for this Submodel is Finished for %s with and Current Time is  %s" % (self , str(system.now_raw())))
                    log.ODM_INFO("==================================")
                    done()
...
class ReconstructionTask(Task):
    ...
    def process_remote(self, done):
        ...
        if not octx.is_feature_matching_done() or not octx.is_reconstruction_done() or self.params['rerun']:
            log.ODM_INFO("==================================")
            log.ODM_INFO("Remote OpenSfm %s with Before Execution Current Time is  %s" % (octx.name(), str(system.now_raw())))
            log.ODM_INFO("==================================")  
            self.execute_remote_task(done, seed_files=["opensfm/exif", 
                                    ...
                                    seed_touch_files=["opensfm/split_merge_stop_at_reconstruction.txt"],
                                     outputs=[
                                            ...
                                            "cameras.json"])
            log.ODM_INFO("==================================")
            log.ODM_INFO("Remote OpenSfm %s with After Execution Current Time is  %s" % (octx.name(), str(system.now_raw())))
            log.ODM_INFO("==================================")  
        else:
...
class ToolchainTask(Task):
    def process_local(self):
        ...
        if not os.path.exists(completed_file) or self.params['rerun']:
            ...
            log.ODM_INFO("==================================")
            log.ODM_INFO("Local ODM Execution %s with Before Execution Current Time is  %s" % (submodels_path, str(system.now_raw())))
            log.ODM_INFO("==================================")  
            system.run(" ".join(map(quote, argv)), env_vars=os.environ.copy())
            log.ODM_INFO("==================================")  
            log.ODM_INFO("Local ODM Execution %s with After Execution Current Time is  %s" % (submodels_path, str(system.now_raw())))
            log.ODM_INFO("==================================") 
    ...
    def process_remote(self, done):
        ...
        def handle_result(error = None):
            ...
            if error is None:
                log.ODM_INFO("==================================")
                log.ODM_INFO("Remote in handle result ODM Execution %s with after Execution Current Time is  %s" % (submodel_name, str(system.now_raw())))
                log.ODM_INFO("==================================")  
                log.ODM_INFO("Process Remote Task Complete handle result")
                self.touch(completed_file)
            done(error=error)

        if not os.path.exists(completed_file) or self.params['rerun']:
            log.ODM_INFO("==================================")
            log.ODM_INFO("Remote ODM Execution %s with Before Execution Current Time is  %s" % (submodel_name, str(system.now_raw())))
            log.ODM_INFO("==================================")  
            ...
            log.ODM_INFO("==================================")
            log.ODM_INFO("Remote ODM Execution %s with after Execution Current Time is  %s" % (submodel_name, str(system.now_raw)))
            log.ODM_INFO("==================================")  
        else:
            log.ODM_INFO("Already processed toolchain for %s" % submodel_name)
            handle_result()